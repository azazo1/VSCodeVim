...
- **Insert 模式支持 Emacs 光标移动.** 打开 `vim.insertModeEmacsBindings` 后, `<C-b>`/`<C-f>` 按字符左右移动, `<C-p>`/`<C-n>` 按行上下移动, `<C-a>`/`<C-e>` 跳到行首/行尾, `<C-d>` 删除光标右侧的字符. 补全列表弹出时 `<C-p>`/`<C-n>` 仍然用于选择补全项. 这套键位会覆盖 Vim 在 Insert 模式下原有的同名绑定, 默认关闭, 详见 `vim.insertModeEmacsBindings` 设置.
