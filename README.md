<h2 align="center"><img src="https://raw.githubusercontent.com/azazo1/VSCodeVim/master/images/icon.png" height="128"><br>VSCodeVim</h2>
<p align="center"><strong>Vim emulation for Visual Studio Code</strong></p>

<!-- TODO: use pgns or something; otherwise vsce won't package it
[![](https://vsmarketplacebadge.apphb.com/version/azazo1.vim.svg)](https://marketplace.visualstudio.com/items?itemName=azazo1.vim)
[![](https://vsmarketplacebadge.apphb.com/installs-short/azazo1.vim.svg)](https://marketplace.visualstudio.com/items?itemName=azazo1.vim)
[![](https://github.com/azazo1/VSCodeVim/workflows/build/badge.svg?branch=master)](https://github.com/azazo1/VSCodeVim/actions?query=workflow%3Abuild+branch%3Amaster)
-->
...
- **Insert 模式支持 Emacs 光标移动.** 打开 `vim.insertModeEmacsBindings` 后, `<C-b>`/`<C-f>` 按字符左右移动, `<C-p>`/`<C-n>` 按行上下移动, `<C-a>` 在行的第一个非空白字符与真正的行首之间切换, `<C-e>` 跳到行尾, `<C-d>` 删除光标右侧的字符. 补全列表弹出时 `<C-p>`/`<C-n>` 仍然用于选择补全项. 这套键位会覆盖 Vim 在 Insert 模式下原有的同名绑定, 默认关闭, 详见 `vim.insertModeEmacsBindings` 设置. 打开 `vim.insertModeEmacsBindingsScreenLine` 后, 上述行边界与上下移动改为按可视行 (折行后的屏幕行) 计算, 箭头键不受影响.

> [!IMPORTANT]
> 本分支 (`azazo1.vim`) 与上游 VSCodeVim (`vscodevim.vim`) 不兼容, 二者共用同一套命令和按键绑定, 同时启用会相互冲突, 请只保留其中一个. 若检测到两者都处于启用状态, 本分支会在激活时弹出警告提示.
...
VSCodeVim can be installed via the VS Code [Marketplace](https://marketplace.visualstudio.com/items?itemName=azazo1.vim) or the OpenVSX [Marketplace](https://open-vsx.org/extension/azazo1/vim).
...
| vim.handleKeys                        | Delegate configured keys to be handled by VS Code instead of by the VSCodeVim extension. Any key in `keybindings` section of the [package.json](https://github.com/azazo1/VSCodeVim/blob/master/package.json) that has a `vim.use<C-...>` in the `when` argument can be delegated back to VS Code by setting `"<C-...>": false`. Example: to use `ctrl+f` for find (native VS Code behavior): `"vim.handleKeys": { "<C-f>": false }`.              | String  | `"<C-d>": true`<br /> `"<C-s>": false`<br /> `"<C-z>": false` |
| vim.insertModeEmacsBindings           | Use Emacs-style cursor movement in Insert mode: `<C-b>`/`<C-f>` move by character, `<C-p>`/`<C-n>` move by line, `<C-a>` toggles between the first non-whitespace character and the beginning of the line, `<C-e>` moves to the end of the line, and `<C-d>` deletes the character to the right of the cursor. While the suggestion widget is open it keeps `<C-p>`/`<C-n>`. These bindings replace the Vim Insert mode bindings on the same keys. | Boolean | false                                                         |
| vim.insertModeEmacsBindingsScreenLine | Measure lines by screen line (a line after word wrap) instead of by logical line for the bindings added by `vim.insertModeEmacsBindings`: `<C-a>`/`<C-e>` move to the beginning/end of the screen line and `<C-p>`/`<C-n>` move by screen line. `<C-a>` still toggles between the first non-whitespace character and the very beginning of that line. Arrow keys are unaffected. Requires `vim.insertModeEmacsBindings`.                           | Boolean | false                                                         |
    ...
    "vim.visualModeKeyBindings": [
        {
            ...
            "commands": [
                {
                    "command": "git.clone",
                    "args": [ "https://github.com/azazo1/VSCodeVim.git" ]
                }
            ]
        }
    ]
...
> :warning: .vimrc support is currently experimental. Only remaps are supported, and you may experience bugs. Please [report them](https://github.com/azazo1/VSCodeVim/issues/new?template=bug_report.md)!
...
| vim.easymotionWordRegex                          | Regex matches for word motions (easymotion w/b/e and delete-mode variants)                               | String  | ``\w+\|[\u4e00-\u9fff]+\|[\u3000-\u303f\uff00-\uffef]+\|[!-/:-@\[-`{-~]+`` |
| vim.easymotionWordMoveMinSpacing                 | Minimum column spacing between adjacent word markers for w/b; set to 1 to show every match               | Number  | 2                                                                          |
...
This project is maintained by a group of awesome [people](https://github.com/azazo1/VSCodeVim/graphs/contributors) and contributions are extremely welcome :heart:. For a quick tutorial on how you can help, see our [contributing guide](/.github/CONTRIBUTING.md).
